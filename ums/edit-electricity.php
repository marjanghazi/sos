...
if (isset($_POST['sale-submit'])) {
    ...
    mysqli_query($conn,"UPDATE ums_bill set station='$station' ,office='$office',provider='$provider',type='$type',consumer_no='$consumer_no',reference_no='$reference_no',consumer_name='$consumer_name',billing_month='$bmonth',reading_date='$reading_date',issuance_date='$issuance_date',due_date='$due_date',previous_reading='$p_reading',current_reading='$c_reading',unit_consumed='$units',start_time='$stime',end_time='$etime',working_days='$days',working_hours='$hours',total_amount='$total_amount',remarks='$remarks' where id_pk='$id'") or die(mysqli_error($conn));
    if ($img_name = $_FILES['img']['size']>0) {
        $img_name = $_FILES['img']['name'];
        $img_temp = $_FILES['img']['tmp_name'];
        move_uploaded_file($img_temp,"assets/images/bills/$img_name");
        mysqli_query($conn,"UPDATE ums_bill set attachment='$img_name' where id_pk='$id'") or die(mysqli_error($conn));
    }
                    ...
                    <div class="col-lg-12">
                        <div class="p-5">
                            ...
                            <form class="user" method="post" action="" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
                                <?php 
                                include 'assets/include/dbconnect.php';
                                $result2 = mysqli_query($conn,"SELECT * from ums_bill where id_pk = '".$_GET['editid']."'") or die(mysqli_error($conn));
                                $row2 = mysqli_fetch_array($result2);
                                ?>
                                <input type="hidden" name="id_pk" value="<?=$_GET['editid'];?>">
                                <div class="form-group row">
                                    <div class="col-sm-4 mb-2 mb-sm-0">
                                        ...
                                        <select class="form-control js-example-basic-single" name="station" required>
                                            ...
                                            <?php
                                            $result = mysqli_query($conn,"SELECT * from ums_stations") or die(mysqli_error($conn));
                                        while ($row = mysqli_fetch_array($result)) {
                                            echo "<option value='".$row['x_code']."' ";
                                            if ($row['x_code'] == $row2['station']) {
                                                echo "SELECTED";
                                            }
                                            echo " >".$row['x_description']."</option>";
                                        }
                                            ?>
                                        </select>
                                    </div>
                                    <div class="col-sm-4 mb-2 mb-sm-0">
                                        ...
                                        <select class="form-control js-example-basic-single" name="office" required>
                                            ...
                                            <?php
                                            $result = mysqli_query($conn,"SELECT * from ums_offices") or die(mysqli_error($conn));
                                        while ($row = mysqli_fetch_array($result)) {
                                            echo "<option value='".$row['id_pk']."' ";
                                            if ($row['id_pk'] == $row2['office']) {
                                                echo "SELECTED";
                                            }
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        while ($row = mysqli_fetch_array($result)) {
                                            echo "<option value='".$row['id_pk']."' ";
                                            if ($row['id_pk'] == $row2['provider']) {
                                                echo "SELECTED";
                                            }
                                            echo " >".$row['provider_name']."</option>";
                                        }
                                            ?>
                                        </select>
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-3">
                                        ...
                                        <select class="js-example-basic-single form-control" name="type" >
                                            <option <?php if ('Single Phase' == $row2['type']) {echo "SELECTED";} ?> >Single Phase</option>
                                            <option <?php if ('Dual Phase' == $row2['type']) {echo "SELECTED";} ?>>Dual Phase</option>
                                            <option <?php if ('Third Phase' == $row2['type']) {echo "SELECTED";} ?>>Third Phase</option>
                                            <option <?php if ('Other' == $row2['type']) {echo "SELECTED";} ?>>Other</option>
                                        </select>
                                    </div>
                                    <div class="col-lg-3">
                                        <div class="form-group" >
                                        ...
                                        <input type="text" class="form-control" name="consumer_no" 
                                            placeholder="Consumer # (optional)" value="<?=$row2['consumer_no']?>" >
                                    ...
                                    <div class="col-lg-3">
                                        ...
                                        <input type="text" class="form-control" name="reference_no" 
                                            placeholder="Reference #" value="<?=$row2['reference_no']?>" required>
                                    ...
                                    <div class="col-lg-3">
                                        ...
                                        <input type="text" class="form-control"name="consumer_name" 
                                            placeholder="Consumer Name" value="<?=$row2['consumer_name'];?>" required>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-3">
                                        ...
                                        <input type="date" class="form-control" value="<?=$row2['reading_date'];?>" required name="reading_date">
                                    </div>
                                    <div class="col-lg-3">
                                        <label class="lbl">Issuance Date</label>
                                        <input type="date" class="form-control" value="<?=$row2['issuance_date'];?>" required name="issuance_date">
                                    </div>
                                    <div class="col-lg-3">
                                        <label class="lbl">Due Date</label>
                                        <input type="date" class="form-control" value="<?=$row2['due_date'];?>" required name="due_date">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-4">
                                        ...
                                        <input type="number" class="form-control" id="pread" value="<?=$row2['previous_reading'];?>" required name="p_reading">
                                    </div>
                                    <div class="col-lg-4">
                                        <label class="lbl">Current Reading</label>
                                        <input type="number" class="form-control" onkeyup="unitscon();" id="cread" value="<?=$row2['current_reading'];?>" required name="c_reading">
                                    </div>
                                    <div class="col-lg-4">
                                        <label class="lbl">Unit Consumed</label>
                                        <input type="number" class="form-control" id="ucon" readonly value="<?=$row2['unit_consumed'];?>" min='1' required name="units">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-3">
                                        ...
                                        <input type="time" value="<?=$row2['start_time'];?>" id="strttime" class="form-control" required name="stime">
                                    </div>
                                    <div class="col-lg-3">
                                        <label class="lbl">Office End Time</label>
                                        <input type="time" value="<?=$row2['end_time'];?>" id="endtime" class="form-control" required name="etime">
                                    </div>
                                    <div class="col-lg-3">
                                        <label class="lbl">Working Days in Month</label>
                                        <input type="number" value="<?=$row2['working_days'];?>" min='1' id="wd" onchange="workingh(this.value);" class="form-control" required name="days">
                                    </div>
                                    <div class="col-lg-3">
                                        <label class="lbl">Total Working Hours</label>
                                        <input type="number" min='1' id="twh" value="<?=$row2['working_hours'];?>" readonly class="form-control" required name="hours">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-3">
                                        ...
                                        <input type="number" min='1' value="<?=$row2['total_amount'];?>" class="form-control" required name="total_amount">
                                    </div>
                                    <div class="col-lg-3">
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-12">
                                        ...
                                        <textarea name="remarks" class="form-control" placeholder="Remarks(Optional)"><?=$row2['remarks'];?></textarea>
                                    </div>
                                </div>
